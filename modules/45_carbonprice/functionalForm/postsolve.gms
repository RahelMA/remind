...
if((cm_iterative_target_adj = 5) OR (cm_iterative_target_adj eq 55) ,  !! End-of-century budget
  s45_actualbudgetco2 = sum(t$(t.val eq 2100),pm_actualbudgetco2(t)); !! why is this not simply pm_actualbudgetco2("2100") ?
else !! Peak budget
...
if((cm_iterative_target_adj eq 4) OR (cm_iterative_target_adj eq 55) ,
  ...
  !! Save pm_taxCO2eq and p45_taxCO2eq_anchor over iterations
  pm_taxCO2eq_iter(iteration,ttot,regi) = pm_taxCO2eq(ttot,regi);

*** 1. Compute deviation from regional target budget 
...
*** 2. Compute rescaling factor 
  if (iteration.val ge 2,
     !! determine whether the carbon price adjustment led to change of emissions in the right direction
      if(cm_iterative_target_adj eq 55,  
        loop(ttot$(ttot.val eq cm_peakBudgYr),
          p45_taxChange(regi) =  pm_taxCO2eq_iter(iteration, ttot, regi) - pm_taxCO2eq_iter(iteration-1, ttot, regi);
            );
        else                                
          p45_taxChange(regi) =  pm_taxCO2eq_iter(iteration, "2100", regi) - pm_taxCO2eq_iter(iteration-1, "2100", regi) ;
      );
      p45_taxChange_iter(iteration, regi) =  p45_taxChange(regi);
      p45_BudgetChange(regi) = p45_actualbudgetco2Regi_iter(iteration, regi) -  p45_actualbudgetco2Regi_iter(iteration - 1, regi);
      p45_BudgetChange_iter(iteration, regi) = p45_BudgetChange(regi);
      p45_ChangeSlope(regi) = p45_taxChange(regi) / p45_BudgetChange(regi); !! if there was no tax change in the previous iteration, this value is zero. If the tax needs to be adjusted, option 2.2 is thus chosen
      p45_ChangeSlope_iter(iteration, regi) = p45_ChangeSlope(regi); 
    );
...
*** 2.1 for later iterations if carbon price adjustment led into the right direction
  if ((iteration.val ge 3) AND (p45_ChangeSlope(regi) lt 0), !! i.e. increase in carbon price led to reduction in budget, or vice versa; i.e. slope was meaningful
        p45_absolutePriceChange(regi) = - pm_budgetDeviation(regi) *  p45_ChangeSlope(regi); !! wanted budget_change * (CP_change / budget_change) !! wanted budget_change = minus (!) the observed deviation
                                                                                              !! (a positive deviation = the carbon budget is too large. Thus, I want the budget to get lower by that amount)
        p45_absolutePriceChange_iter(iteration, regi) = p45_absolutePriceChange(regi);
         !! the following could become negative when pm_taxCO2eq < p45_absolutePriceChange. This is the case when the carbon-price-sensitivity is very low compared to the required budget change(for whatever reason). 
         !! Leaving it for now, because the funnel takes care of it.
        p45_newPrice(regi) = pm_taxCO2eq("2100", regi) + p45_absolutePriceChange(regi);
        p45_newPrice_iter(iteration, regi) = p45_newPrice(regi);           
        p45_factorRescale_taxCO2Regi(iteration, regi) =  (pm_taxCO2eq("2100", regi) + p45_absolutePriceChange(regi)) / pm_taxCO2eq("2100", regi);     
   
*** 2.2 for earlier iterations or if carbon price adjustment led into the wrong direction
    ...
    if (pm_budgetCO2from2020Regi(regi) > 0,   
          ...
          if (p45_actualbudgetco2Regi(regi) >= 0,
            p45_factorRescale_taxCO2Regi(iteration, regi) = 
               max(0.5, (p45_actualbudgetco2Regi(regi) / pm_budgetCO2from2020Regi(regi))); 
        
          !! a1) negative actual budget                     
          else 
          !! ratio = (positive target) / (absolute difference). Potential problem: Approaches 1 for small negative budgets
            p45_factorRescale_taxCO2Regi(iteration, regi) = 
                max(0.5, (pm_budgetCO2from2020Regi(regi) /               
                              (pm_budgetCO2from2020Regi(regi) + abs(p45_actualbudgetco2Regi(regi)))
                        )) ;
          )  !! positive target             
    ...
    !! b) negative budget target
      ...
      !! b1) positive actual budget             => take the absolute deviation & rescale to get a factor: ratio = log(actual + asb(target))
           if (p45_actualbudgetco2Regi(regi) >= 0,
             p45_factorRescale_taxCO2Regi(iteration, regi) = 
                log(abs(pm_budgetCO2from2020Regi(regi)) + p45_actualbudgetco2Regi(regi)) 
          !! b2) negative actual budget         => case reverse to global target default case: ratio = (target/actual budget)
          else
            p45_factorRescale_taxCO2Regi(iteration, regi) = 
                 max(0.5, (pm_budgetCO2from2020Regi(regi) / p45_actualbudgetco2Regi(regi))))
          )  !! target type
...
*** 3. add a funnel to avoid excessive adjustments. 
  ...
  loop(regi,
     p45_factorRescale_taxCO2Regi_Funneled(iteration, regi)                                        
        = max(min( cm_funnelFactor * EXP( -cm_funnelExponent * iteration.val) + 1 + cm_funnelLower, p45_factorRescale_taxCO2Regi(iteration,regi)), !! a) a maximum adjustment value which decreases with the number of iterations
...
!! 2.1. get the carbon price trajectory used in this iteration
if((cm_EOCstartShape eq 1) AND (iteration.val eq 1), !! the global anchor trajectory is chosen as starting point after iteration 1 -> all regions have the same shape
      p45_taxCO2eq_anchorRegi(ttot, regi)$(ttot.val gt 2005) = p45_taxCO2eq_anchor(ttot); 
    else                                             !! For all iterations > 1 and if a regional shape should be chosen in iteration 1
      p45_taxCO2eq_anchorRegi(ttot, regi)$(ttot.val gt 2005) = pm_taxCO2eq(ttot,regi);  !! get the CO2 tax that was used in this iteration, to be adjusted
    );

!! Option A: slope of CP is not adjusted --> entire trajectory is rescaled, values will be used as of cm_startYear (see below)
if(cm_CPslopeAdjustment = 0, 
    !! A2.2. Scale the anchor trajectory in this iteration to get the adjusted trajectory to be used in iteration+1 
    p45_taxCO2eq_anchorRegi(ttot,regi)$(ttot.val gt 2005) = 
        p45_taxCO2eq_anchorRegi(ttot, regi) * p45_factorRescale_taxCO2Regi_Funneled(iteration,regi);

    !! Adjust the shape if the peak-budget carbon price shape is set
    if(cm_iterative_target_adj = 55 , !! After cm_peakBudgYr, the global anchor trajectory increases linearly with fixed annual increase given by cm_taxCO2_IncAfterPeakBudgYr
...
!! Option 2B: slope of CP is adjusted
if(cm_CPslopeAdjustment = 1, 
    ...
    !! B2.2: Save the carbon price. The values prior to cm_startYear will not be adjusted    
    p45_taxCO2eq_anchorRegi(ttot,regi)$(ttot.val ge 2005) = pm_taxCO2eq(ttot,regi);

    !! Option (a): If peak-budget shape:
    if(cm_iterative_target_adj = 55, 
    !! B2.3a: Set the rescaled anchor trajectory as of cm_peakBudgYr
      p45_taxCO2eq_anchorRegi(ttot,regi)$(ttot.val eq cm_peakBudgYr) = 
                  p45_temp_anchor(ttot,regi) * p45_factorRescale_taxCO2Regi_Funneled(iteration,regi);
    ...
    !! B2.4a: Calculate the slope for a linear connection between the last carbon price from input data and the Price in the Peak Budget year
...
!! 2.4. Always set carbon price constant after 2100 to prevent huge taxes after 2100 and the resulting convergence problems
p45_taxCO2eq_anchorRegi(ttot,regi)$(ttot.val gt 2100) = p45_taxCO2eq_anchorRegi("2100",regi);

!! 2.5. assign the value to pm_taxCO2eq that will be used in the next iteration
pm_taxCO2eq(ttot,regi)$(ttot.val ge cm_startyear) = p45_taxCO2eq_anchorRegi(ttot,regi);

!! save final chosen trajectory
p45_taxCO2eq_anchorRegi_iter(ttot, regi, iteration)  = p45_taxCO2eq_anchorRegi(ttot, regi);
!! regional targets adjustment
...
*** ----- DETERMINES IF PART I AND II ARE RUN IN THE FOLLOWING ----- 
...
if((cm_iterative_target_adj eq 5) OR (cm_iterative_target_adj eq 9),
  ...
  else !! less sensitivity of CO2 price adjustment to CO2 budget deviation for end of century budget targets
  ...
  if( (o_modelstat ne 2) OR (abs(sm_globalBudget_absDev) le cm_budgetCO2_absDevTol) OR (ord(iteration) = cm_iteration_max), 
    ...
    if (s45_actualbudgetco2 > 0, !! if budget positive
      ...
      p45_factorRescale_taxCO2_Funneled(iteration)                                                !! To avoid 
        = max(min( 2 * EXP( -0.15 * iteration.val ) + 1.01, p45_factorRescale_taxCO2(iteration)), !! a) a maximum adjustment value which decreases with the number of iterations
              1/ ( 2 * EXP( -0.15 * iteration.val ) + 1.01)                                       !! b) a minimum adjustment value which increases with the number of iterations (0.95 for iter 25)
          );
